...
<?php
  ...
  if($ext <>'csv') {
	  ...
	  if (($handle = fopen($_FILES['fcsv']['tmp_name'], "r")) !== FALSE) { 
		$import = "insert into gudang (kode,gudang,info) values ";
		while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
			if($data[0] <>'kode') {
				$import .= "('$data[0]','$data[1]','$data[2]'),";
			}
		}
		$import .=",";
	  }
	  ...
	  if(!$result) {
		...
		echo "<script type=\"text/javascript\">setTimeout(\"location.href='index-c-lokasigudang-t-add-k-importcsv.pos'\", 2000);</script>";
	  }
  }
?>
</div>
<?php } ?>
<h1>Import Lokasi Gudang (CSV)</h1> 
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="4">
    <tr>
      ...
      <td width="82%"><label>
        ...
        [ <a href="formatcsv/lokasigudang.csv">Format data</a> ]</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
