<?php
	...
	//Define variable
    $stat = '';
	$url = "index-c-jeniskontak.pos";
	$p   = $_GET['p'];
	$id  = $_POST['id'];
	$kod = strtoupper(str_replace(" ","_",$_POST['kode']));
	$kla = $_POST['klasifikasi'];
	...
	if($p <>'mdelete' AND $p <>'delete') {
		if($p=='add') {
			// cek username
			$query_cek = "select kode from jeniskontak where kode='$kod'";
...
//		echo "<img src=\"images/alert.gif\" hspace=\"5\"/><b style=\"color:#FA5121;\">Kesalahan : </b><br />".implode("<br />", $error);
		...
		switch($p) {
			...
			$query_exe = "delete from jeniskontak".$where;
...
//					echo "<center><img src=\"images/alert.gif\" hspace=\"5\"/><b style=\"color:#FA5121;\">Data gagal dihapus, coba lagi !!!</b></center>";
			...
			break;
			case("edit"):
			$query_exe = "update jeniskontak set klasifikasi='$kla', jenis='$jen', info='$inf' where kode='$kod'";
			$exe = $mysqli->query($query_exe);
			if($exe) {
