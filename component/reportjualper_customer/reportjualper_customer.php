...
$bp = $b * $p;
//----
$query_data  = "select b.kode, b.kontak, count(a.referensi) as total_ec, sum(a.total) as total_sales from keluarbarang a, kontak b where a.client = b.kode";
//----
...
//---- filter data*/
if($q <>'') {
	$query_data .= " AND (b.kontak LIKE '%$q%' OR b.kode LIKE '%$q%' OR a.sales LIKE '%$q%')";
	$query_all   = $query_data;
}
//----
$query_data .= " group by b.kode,b.kontak limit $b offset $bp";
$data = $mysqli->query($query_data) or die(mysql_error());
$row_data = mysqli_fetch_assoc($data);
//---
$query_all .=" group by b.kode,b.kontak";
...
?>
<div class="tablebg">
  <h1 class="hide">Rekap Sales Monthly</strong> ( Customer )</h1>
  ...
  <table class="datatable-print" width="700" border="0" cellspacing="0" cellpadding="3">
    <tr>
      ...
      <tr>
        ...
        <td align="center"><strong class="titleheader">REKAP SALES MONTHLY PER CUSTOMER</strong><br />
  ...
  <br />
        ...
        <td width="15%" align="center"><strong>Rekap<br />
          Penjualan per Customer</strong></td>
      </tr>
      <tr>
    ...
    <tr valign="top">
      ...
      <td align="right"><?php echo number_format($row_data['total_ec'],0,',','.');?></td>
      <td align="right"><?php echo number_format($row_data['total_sales'],0,',','.');?></td>
      </tr>
