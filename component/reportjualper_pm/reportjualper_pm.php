...
$bp = $b * $p;
//----
$query_data  = "select a.kode, a.kontak, b.jenis, sum(c.total) as actual_sales, count(c.referensi) as sumref, count(c.client) as sumclient , sum(d.target) as target from kontak a, jeniskontak b, keluarbarang c, targetpm d where a.jenis = b.kode AND b.klasifikasi='sales' AND a.kode = c.sales AND d.kontak = c.sales";
//----
...
// ---
$query_data .= " AND c.tgl >='$awal_period' AND c.tgl <='$akhir_period'";
$query_all  .= " AND c.tgl >='$awal_period' AND c.tgl <='$akhir_period'";
/*// ----
	$query_data .= " AND a.tgl >='$q__-$q_-01' AND a.tgl <='$q__-$q_-$last_tgl'";
//---- filter data*/
if($q <>'') {
	$query_data .= " AND (a.kode LIKE '%$q%' OR a.kontak LIKE '%$q%' OR b.jenis LIKE '%$q%')";
	$query_all   = $query_data;
}
//----
$query_data .= " group by a.kode,a.kontak limit $b offset $bp";
$data = $mysqli->query($query_data) or die(mysql_error());
$row_data = mysqli_fetch_assoc($data);
//---
$query_all .=" group by a.kode,a.kontak";
...
?>
<div class="tablebg">
  <h1 class="hide">Rekap Sales Monthly</h1>
  ...
  <table class="datatable-print" width="700" border="0" cellspacing="0" cellpadding="3">
    <tr>
      <td colspan="7" style="border-top:solid 1px #000;border-left:solid 1px #000;"><table width="100%" border="0" cellpadding="2" cellspacing="0" id="head-report">
      <tr>
        <td width="15%" align="center" valign="top"><img src="images/chi_logo.jpg" hspace="0" vspace="0" /></td>
        <td align="center"><strong class="titleheader">REKAP SALES MONTHLY / Sales</strong><br />
  ...
  <br />
    ...
    <tr valign="top">
      ...
      <td align="right"><?php echo $row_data['sumclient'];?></td>
      <td align="right"><?php echo $row_data['sumref'];?></td>
      <td align="right"><?php echo number_format($row_data['actual_sales'],0,',','.');?></td>
      <td align="right"><?php echo round(($row_data['actual_sales'] / $row_data['target']) * 100,2);?></td>
      </tr>
      <?php if($no%30==0 AND $no <> 30) { ?>
      <tr>
        <td colspan="7" class="noborder" height="25px"><br /></td>
    </tr>
      <tr>
        <td colspan="7" style="border-left:solid 1px #000;border-top:solid 1px #000;"><table width="100%" border="0" cellpadding="2" cellspacing="0" id="head-report">
      <tr>
        <td width="15%" align="center" valign="top"><img src="images/chi_logo.jpg" hspace="0" vspace="0" /></td>
        <td align="center"><strong class="titleheader">REKAP SALES MONTHLY / Sales</strong><br />
          <span style="text-transform:uppercase;font-size:13px;letter-spacing:1px;">Solusi cepat untuk masalah Bisnis &amp; usaha anda<br/>
    ...
    <?php }else{ ?>
    <tr>
      <td colspan="7" style="border-left:solid 1px #000;">Data tidak ada</td>
    </tr>
    <?php } ?>
  </table>
</div>
