...
$id = $_GET['id'];
// query edit
$query_edit = "select a.noreferensi, a.qty, b.product_id, b.barang, c.kode as sid, c.kode as sid, c.satuan, d.jenis from dmasukbarang a join barang b on a.barang = b.kode join satuan c on a.satuan = c.kode join jenisbarang d on b.jenis=d.kode where a.id='$id'";
$edit = $mysqli->query($query_edit);
$row_edit = mysqli_fetch_assoc($edit);
$total_edit = mysqli_num_rows($edit);
// terima barang
$query_terbrg = "select sum(qty) as terima from terimabarang where noreferensi='$row_edit[noreferensi]' AND barang='$row_edit[kode]' AND satuan='$row_edit[sid]'";
$terbrg       = $mysqli->query($query_terbrg);
$row_terbrg   = mysqli_fetch_assoc($terbrg);
// retur barang
$query_retur  = "select sum(qty) as retur from terimabarang_r where noreferensi='$row_edit[noreferensi]' AND barang='$row_edit[kode]' AND satuan='$row_edit[sid]'";
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="4">
    ...
    <tr valign="top">
      ...
      <td><?php echo $row_edit['kode'];?> - <?php echo $row_edit['jenis'];?> - <?php echo $row_edit['barang'];?><label>
        <input name="total" type="text" id="total" value="<?php echo intval($row_edit['qty'] - $terima);?>" size="8" maxlength="10" />
      </label>
