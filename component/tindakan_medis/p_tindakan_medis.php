<?php
	...
	//Define variable
	$url = "index.php?component=tindakan_medis";
	$p   = $_GET['p'];
	$id  = $_POST['id'];
	$kod = strtoupper(str_replace(" ","_",$_POST['kode']));
	$tme = $mysqli->real_escape_string($_POST['tmedis']);
	$bia = $_POST['biaya'];
	...
	if($p <>'mdelete' AND $p <>'delete') {
		if($p=='add') {
			// cek username
			$query_cek = "select kode from hos_tmedis where kode='$kod'";
		...
		if (trim($tme) == '') {
			$error[] = '- Tindakan Medis harus diisi !!!';
		}
		if (!is_numeric($bia)) {
			$error[] = '- Biaya harus angka !!!';
		}
	}else if($p =='mdelete'){
		if($jdata <= 0) {
...
//		echo "<img src=\"images/alert.gif\" hspace=\"5\"/><b style=\"color:#FA5121;\">Kesalahan : </b><br />".implode("<br />", $error);
		...
		switch($p) {
			...
			$query_exe = "delete from hos_tmedis".$where;
...
//					echo "<center><img src=\"images/alert.gif\" hspace=\"5\"/><b style=\"color:#FA5121;\">Data gagal dihapus, coba lagi !!!</b></center>";
                    $stat = 'Data gagal diapus, coba lagi !!!';
				}
			break;
			case("edit"):
			$query_exe = "update hos_tmedis set tmedis='$tme', biaya='$bia', info='$inf' where kode='$kod'";
			$exe = $mysqli->query($query_exe);
			if($exe) {
...
//					echo "<center><img src=\"images/alert.gif\" hspace=\"5\"/><b style=\"color:#FA5121;\">Data gagal disimpan, coba lagi !!!</b></center>";
			...
			break;
			default:
				$query_exe = "insert into hos_tmedis values ('$kod','$tme','$bia','$inf')";
				$exe = $mysqli->query($query_exe);
				//echo $query_exe;
				if($exe) {
//					echo "<center><img src=\"images/_info.png\" hspace=\"5\"/><b style=\"color:#1A4D80;\">Data telah disimpan ...</b></center>";
