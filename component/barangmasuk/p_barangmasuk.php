<?php
	...
	if (isset($error)) {
		...
		switch($p) {
			...
			$query_exe = "delete from terimabarang".$where;
...
//					echo "<center><img src=\"images/alert.gif\" hspace=\"5\"/><b style=\"color:#FA5121;\">Data gagal dihapus, coba lagi !!!</b></center>";
			...
			break;
			case("edit"):
			$query_exe = "update terimabarang set tgl='$tgl', qty='$qty', info='$inf' where id='$id'";
			$exe = $mysqli->query($query_exe);
			if($exe) {
					// adjustment stok
					$adj_value = $qty - $_POST['qtyold'];
					$stoknow   = $_POST['stok'] + $adj_value;
					// update stok
