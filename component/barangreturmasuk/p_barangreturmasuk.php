<?php
	...
	//Define variable
    $stat = '';
	$url = "index-c-barangreturmasuk.pos";
	$p   = $_GET['p'];
	$id  = $_POST['id'];
	$tgl = $_POST['tgl'];
	$qty = $_POST['qty'];
	...
	if (isset($error)) {
		...
		switch($p) {
			...
			$query_exe = "delete from terimabarang_r".$where;
...
//					echo "<center><img src=\"images/alert.gif\" hspace=\"5\"/><b style=\"color:#FA5121;\">Data gagal dihapus, coba lagi !!!</b></center>";
			...
			break;
			case("edit"):
				$old_processed = $_POST['old_processed'];
				$processed = $_POST['processed'];
				$processed_info = $mysqli->real_escape_string($_POST['processed_info']);
				
				$query_exe = "UPDATE terimabarang_r SET tgl='$tgl', qty='$qty', info='$inf', processed='$processed', processed_info='$processed_info' WHERE id = $id";
				$exe = $mysqli->query($query_exe);
				if($exe)
				{
					// adjustment stok
					...
					//$mysqli->query("update stokbarang set qty='$stoknow' where id='$_POST[stokid]'");
					
					if ($old_processed == "false" && $processed == "true")
					{
						// update qty barang
						$mysqli->query("UPDATE barang SET qty=qty+$qty WHERE product_id=$_POST[product_id]");
					}
					else if ($old_processed == "true" && $processed == "false")
					{
						// update qty barang
						$mysqli->query("UPDATE barang SET qty=qty-$qty WHERE product_id=$_POST[product_id]");
					}
